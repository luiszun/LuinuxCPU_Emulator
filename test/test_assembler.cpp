...
public:
  bool HasPendingLiteral() { return _pendingLiteralValue; }
  uint16_t GetPendingLiteral() { return _literalValue; }
...
TEST(TestAssemblerSuite, TestStringLiteral) {
  ...
  ASSERT_EQ(asmObj.GetValueFromStringLiteral("-10"),
...
TEST(TestAssemblerSuite, TestMultilineProgram) {
  ...
  std::vector<uint8_t> expectedBinary{0x76, 0x25, 0x01, 0x00, 0x76,
                                      0x26, 0x00, 0x01, 0x45, 0x67};
...
TEST(TestAssemblerSuite, TestLoop10x) {
  ...
  std::string program =
      ...
      "JNZ R1, R2\n"
      "STOP";
  std::vector<uint8_t> expectedBinary{0x76, 0x25, 0x00, 0x0a, 0x76, 0x2f, 0x00,
                                      0x00, 0x76, 0x27, 0x00, 0x0c, 0x76, 0x8f,
  ...
  ASSERT_STREQ(str.c_str(),
               "\\x76\\x25\\x00\\x0a\\x76\\x2f\\x00\\x00\\x76\\x27\\x00\\x0c\\x"
               "76\\x8f\\x15\\xf6\\x71\\x67\\x76\\x91");
  ASSERT_EQ(expectedBinary, binProgram);
}
...
TEST(TestAssemblerSuite, TestAddressTags) {
  ...
  std::vector<uint8_t> expectedBinary{
      0x76, 0x25, 0x00, 0x0a, 0x76, 0x2f, 0x00, 0x00, 0x76, 0x27, 0x00, 0x0c,
      0x76, 0x8f, 0x15, 0xf6, 0x71, 0x67, 0x76, 0x91, 0x76, 0x25, 0x00, 0x10};
  ...
  ASSERT_STREQ(str.c_str(),
               "\\x76\\x25\\x00\\x0a\\x76\\x2f\\x00\\x00\\x76\\x27\\x00\\x0c\\x"
               "76\\x8f\\x15\\xf6\\x71\\x67\\x76\\x91\\x76\\x25\\x00\\x10");
  ASSERT_EQ(expectedBinary, binProgram);
}