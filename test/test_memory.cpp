...
public:
  TestNVMemory(size_t size, std::string filename)
      : NVMemory<uint16_t>(size, filename) {}

  void FlushOut() { _FlushOut(); }
  void FlushIn() { _FlushIn(); }
};
...
TEST(TestMemorySuite, TestNonVolatileMemoryReadWrite) {
  ...
  std::array<uint8_t, 17> memdmp = {0xde, 0xad, 0xbe, 0xef, 0x0,  0x0,
                                    0x0,  0x0,  0xc0, 0x8b, 0xad, 0xf0,
                                    0x0d, 0x00, 0x00, 0x00, 0x00};
  // Last 4 bytes are different
  std::array<uint8_t, 17> memdmp2 = {0xde, 0xad, 0xbe, 0xef, 0x0,  0x0,
                                     0x0,  0x0,  0xc0, 0x8b, 0xad, 0xf0,
