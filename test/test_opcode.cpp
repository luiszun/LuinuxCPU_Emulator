...
TEST(TestOpCodesSuite, TestOverlappingOpcodes) {
  ...
  for (auto i = mnemonicTable.begin(); i != mnemonicTable.end(); ++i) {
    ...
    std::cout << std::hex << opName << " 0x" << std::setfill('0')
              << std::setw(4) << opCodeStart << " 0x" << std::setfill('0')
              << std::setw(4) << nextOpCode - 1 << std::endl;
  ...
  for (auto i = 0; i < 0xffff; ++i) {
    if (!gotBeginning) {
      if (usedOpcode[i] != 0)
        continue;
      gotBeginning = true;
      std::cout << "Free opcode from 0x" << std::hex << std::setfill('0')
                << std::setw(4) << i;
    } else {
      if (usedOpcode[i] == 0)
        continue;
      gotBeginning = false;
      std::cout << " to 0x" << std::hex << std::setfill('0') << std::setw(4)
                << i - 1 << std::endl;
    }
  }
