...
const char loop10xShellCode[] =
    "\x76\x25\x00\x10\x76\x26\x00\x01\x45\x67\x76\x25\x00\x0e";
...
public:
  ...
  void FetchInstruction() { _FetchInstruction(); }
  void DecodeInstruction() { _DecodeInstruction(); }
  void ExecuteInstruction() { _ExecuteInstruction(); }

  uint16_t GetFetchedInstruction() { return _fetchedInstruction; }

  OpCodeId GetDecodedOP() { return _decodedOpCodeId; }

  RegisterId GetArgs(unsigned argN) { return _instructionArgs[argN]; }

  std::unordered_map<RegisterId, Register> &GetRegisters() {
    return _registers;
  }

  Memory16 &GetMainMemory() { return *_mainMemory; }
...
TEST(TestProcessorPrograms, TestAluFlags_ADD) {
  Assembler asmObj;
  ...
  std::cout << "Assembled Program (Hex Escaped): " << escapedProgram
            << std::endl;
...
TEST(TestProcessorPrograms, TestBitOps) {
  ...
  ASSERT_EQ(cpu.ReadRegister(RegisterId::RFL) &
                static_cast<uint16_t>(FlagsRegister::Zero),
            1);
}
...
TEST(TestProcessorPrograms, TestSWM) {
  ...
  std::shared_ptr<NVMemory16> nvram =
      std::make_shared<NVMemory16>(0x10000, "test_nvmemory.bin");
...
TEST(TestProcessorPrograms, TestJMP) {
  Assembler asmObj;
  std::string program =
      "; This program simply finds the last zeroe'd address and writes its "
      "address onto that corresponding address\n"
      "SET R0, h'fffe ; R0 = address of the last zero'ed address, fffe is the "
      "last 16 bit valid address\n"
...
TEST(TestProcessorPrograms, TestJE) {
  Assembler asmObj;
  std::string program = "; This program force tests JE and JNE by following "
