...
public:
  ...
  void WritePayload(TAddressSpace address, std::vector<uint8_t> payload) {
    ...
    // This will help us catch if the CPU tries to fetch non-payload
    // instructions
...
template <typename TAddressSpace>
class NVMemory : public Memory<TAddressSpace> {
public:
  NVMemory(size_t size, std::string filename)
      : Memory<TAddressSpace>(size), _filename(filename) {
    _FlushIn();
  }
...
protected:
  void _FlushOut() {
    ...
    _outFile.write(reinterpret_cast<char *>(&(this->_memory[0])),
                   this->_memory.size());
    _outFile.close();
  }
  ...
  void _FlushIn() {
    ...
    _inFile.read(reinterpret_cast<char *>(&(this->_memory[0])),
                 this->_memory.size());
    _inFile.close();
  }
...
template <typename TAddressSpace>
class NonByteAddressableMemory : public Memory<TAddressSpace> {
public:
