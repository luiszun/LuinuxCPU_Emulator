...
Processor::Processor(Memory16 &programMemory, std::shared_ptr<NVMemory16> nvram)
    : _programMemory(programMemory), _nvram(nvram),
      _internalMemory(InternalMemorySize) {
  ...
  for (auto i = RegisterId::RAC; i != RegisterId::END_OF_REGLIST;
       i = RegisterId(static_cast<uint8_t>(i) + 1)) {
    _registers.insert({i, Register(static_cast<uint8_t>(i) * uint8_t{2},
                                   _internalMemory, i)});
  }
...
void Processor::_DecodeInstruction() {
  ...
  if ((_decodedOpCodeId != OpCodeId::INVALID_INSTR) ||
    ...
    throw std::runtime_error(
        "Decoding new instruction with previous exec cycle unfinished.");
  }

  // We check this case for leading zeroes, the rest of instructions can be
  // determined below.`
  ...
  if ((_decodedOpCodeId == OpCodeId::SET) ||
      _decodedOpCodeId == OpCodeId::SET_M) {
    ...
    // int i because we want to know when it reaches -1 as opposed to it running
    // amok by rollover
    ...
    if (_fetchedInstruction != opCodeTable.at(_decodedOpCodeId).opCode) {
      throw std::runtime_error("Something went wrong. Did we decode more or "
...
void Processor::_ExecuteInstruction() {
  // Let's create a table of function pointers to the instructions. That way we
  ...
  static const std::unordered_map<OpCodeId, OpFunction> opCodeFunctionTable = {
      {OpCodeId::ADD, &Processor::ADD},
      {OpCodeId::SUB, &Processor::SUB},
      {OpCodeId::MUL, &Processor::MUL},
      {OpCodeId::DIV, &Processor::DIV},
      {OpCodeId::AND, &Processor::AND},
      {OpCodeId::OR, &Processor::OR},
      {OpCodeId::XOR, &Processor::XOR},
      {OpCodeId::JZ, &Processor::JZ},
      {OpCodeId::JNZ, &Processor::JNZ},
      {OpCodeId::MOV, &Processor::MOV},
      {OpCodeId::JE, &Processor::JE},
      {OpCodeId::JNE, &Processor::JNE},
      {OpCodeId::TSTB, &Processor::TSTB},
      {OpCodeId::SETZ, &Processor::SETZ},
      {OpCodeId::SETO, &Processor::SETO},
      {OpCodeId::SET, &Processor::SET},
      {OpCodeId::PUSH, &Processor::PUSH},
      {OpCodeId::POP, &Processor::POP},
      {OpCodeId::NOT, &Processor::NOT},
      {OpCodeId::SHFR, &Processor::SHFR},
      {OpCodeId::SHFL, &Processor::SHFL},
      {OpCodeId::INC, &Processor::INC},
      {OpCodeId::DEC, &Processor::DEC},
      {OpCodeId::NOP, &Processor::NOP},
      {OpCodeId::STOP, &Processor::STOP},
      {OpCodeId::ADD_RM, &Processor::ADD_RM},
      {OpCodeId::ADD_MR, &Processor::ADD_MR},
      {OpCodeId::ADD_MM, &Processor::ADD_MM},
      {OpCodeId::SUB_RM, &Processor::SUB_RM},
      {OpCodeId::SUB_MR, &Processor::SUB_MR},
      {OpCodeId::SUB_MM, &Processor::SUB_MM},
      {OpCodeId::MUL_RM, &Processor::MUL_RM},
      {OpCodeId::MUL_MR, &Processor::MUL_MR},
      {OpCodeId::MUL_MM, &Processor::MUL_MM},
      {OpCodeId::DIV_RM, &Processor::DIV_RM},
      {OpCodeId::DIV_MR, &Processor::DIV_MR},
      {OpCodeId::DIV_MM, &Processor::DIV_MM},
      {OpCodeId::AND_RM, &Processor::AND_RM},
      {OpCodeId::AND_MR, &Processor::AND_MR},
      {OpCodeId::AND_MM, &Processor::AND_MM},
      {OpCodeId::OR_RM, &Processor::OR_RM},
      {OpCodeId::OR_MR, &Processor::OR_MR},
      {OpCodeId::OR_MM, &Processor::OR_MM},
      {OpCodeId::XOR_RM, &Processor::XOR_RM},
      {OpCodeId::XOR_MR, &Processor::XOR_MR},
      {OpCodeId::XOR_MM, &Processor::XOR_MM},
      {OpCodeId::JZ_RM, &Processor::JZ_RM},
      {OpCodeId::JZ_MR, &Processor::JZ_MR},
      {OpCodeId::JZ_MM, &Processor::JZ_MM},
      {OpCodeId::JNZ_RM, &Processor::JNZ_RM},
      {OpCodeId::JNZ_MR, &Processor::JNZ_MR},
      {OpCodeId::JNZ_MM, &Processor::JNZ_MM},
      {OpCodeId::MOV_RM, &Processor::MOV_RM},
      {OpCodeId::MOV_MR, &Processor::MOV_MR},
      {OpCodeId::MOV_MM, &Processor::MOV_MM},
      {OpCodeId::TSTB_M, &Processor::TSTB_M},
      {OpCodeId::SETZ_M, &Processor::SETZ_M},
      {OpCodeId::SETO_M, &Processor::SETO_M},
      {OpCodeId::SET_M, &Processor::SET_M},
      {OpCodeId::PUSH_M, &Processor::PUSH_M},
      {OpCodeId::POP_M, &Processor::POP_M},
      {OpCodeId::NOT_M, &Processor::NOT_M},
      {OpCodeId::SHFR_M, &Processor::SHFR_M},
      {OpCodeId::SHFL_M, &Processor::SHFL_M},
      {OpCodeId::INC_M, &Processor::INC_M},
      {OpCodeId::DEC_M, &Processor::DEC_M},
  ...
  for (size_t i = 0; i < _instructionArgs.size(); ++i) {
    argumentsAsRegisters.push_back(
        std::make_shared<Register>(_registers.at(_instructionArgs.at(i))));
  }

  LuinuxAssert(
      opCodeFunctionTable.count(_decodedOpCodeId) > 0,
...
ConstantPair
Processor::_Get_RR(std::vector<std::shared_ptr<Register>> args) const {
  auto opA = args.at(0)->Read();
  auto opB = args.at(1)->Read();
  return std::make_pair(opA, opB);
}
ConstantPair
Processor::_Get_MR(std::vector<std::shared_ptr<Register>> args) const {
  auto opA = _DereferenceRegisterRead(args.at(0)->registerId);
  auto opB = args.at(1)->Read();
  return std::make_pair(opA, opB);
}
ConstantPair
Processor::_Get_RM(std::vector<std::shared_ptr<Register>> args) const {
  auto opA = args.at(0)->Read();
  auto opB = _DereferenceRegisterRead(args.at(1)->registerId);
  return std::make_pair(opA, opB);
}
ConstantPair
Processor::_Get_MM(std::vector<std::shared_ptr<Register>> args) const {
  auto opA = _DereferenceRegisterRead(args.at(0)->registerId);
  auto opB = _DereferenceRegisterRead(args.at(1)->registerId);
  return std::make_pair(opA, opB);
}
...
void Processor::ADD_MR(std::vector<std::shared_ptr<Register>> args) {
  ...
  std::shared_ptr racPtr =
      std::make_shared<Register>(_registers.at(RegisterId::RAC));
...
void Processor::POP_M(std::vector<std::shared_ptr<Register>> args) {
  ...
  _DereferenceRegisterWrite(args.at(0)->registerId,
                            _DereferenceRegisterRead(RSP.registerId));
}
...
void Processor::SWM(std::vector<std::shared_ptr<Register>> args) {
  ...
  if (0 == f.flags.Memory) {
    ...
    if (_nvram == nullptr) {
      throw std::runtime_error(
          "Trying to use NVRAM, but it was not prepared on this setup.");
    }
    _mainMemory = _nvram;
  }
}
